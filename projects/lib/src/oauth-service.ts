...
export class OAuthService extends AuthConfig implements OnDestroy {
    ...
    public tryLoginCodeFlow(options: LoginOptions = null): Promise<void> {
        ...
        if (code) {
            this.storeAuthorizationCodeInternal(code);

            if (!this.skipAuthorizationCodeExchange) {
                return new Promise((resolve, reject) => {
    ...
    public getAuthorizationCode(): string {
        return this._storage
            ? this._storage.getItem('authorization_code')
            : null;
    }

    protected storeAuthorizationCodeInternal(code: string): void {
        if (this._storage && this.storeAuthorizationCode) {
            this._storage.setItem('authorization_code', code);
        }
    }
